...
#SBATCH --job-name=neox_65b_4M_v2
...
## MODEL
model_type=gpt_neox
SCRIPT_ARGS+="--model_type $model_type --model_size 65b "

## HYPERPARAMS
SCRIPT_ARGS+="--lr 0.0004 "
SCRIPT_ARGS+="--weight_decay 0.1 "
...
## RESUME
# SCRIPT_ARGS+="--resume_from_checkpoint checkpoints/$SLURM_JOB_NAME/$model_type-10steps "

CODE_DIR=`pwd`
srun -l bash -c "cd $CODE_DIR && conda run -p $CONDA_ENV_PATH --no-capture-output $SHELL_SCRIPT --hostfile $HOSTFILE $SCRIPT_ARGS"
