...
# First Party
from .core import get_tp_process_group, local_rank, tp_rank, tp_size

InvalidSeqLenPrescaledBatchError = RuntimeError
PaddingSizeError = RuntimeError
ShiftValueError = RuntimeError
SMPInvalidArgumentError = RuntimeError
SplitShapeLenError = RuntimeError
SplitShapeMismatchError = RuntimeError
...
def parameter_creation_scope(
    ...
    existing_modules = {m for m in module.modules()}

    yield

    for p in module.parameters():
        if scaled_batch and p not in existing_params:
            core.module_manager.add_scaled_batch_parameter(p)

    for b in module.buffers():
        if scaled_batch and b not in existing_buffers:
            core.module_manager.add_scaled_batch_buffer(b)

    for m in module.modules():
        if use_normal and m not in existing_modules:
            override_params_with_normal(m, initializer_range)
...
def initialize_with_input_partition(
    ...
    for n, p in module.named_parameters():
        if p not in existing_params and n not in exclude_from_dist:
            core.module_manager.add_distributed_parameter(p, axis)
        elif n in exclude_from_dist:
            core.module_manager.add_one_rank_parameter(p)

    for n, b in module.named_buffers():
        if b not in existing_buffers and n not in exclude_from_dist:
            core.module_manager.add_distributed_buffer(b, axis)
        elif n in exclude_from_dist:
            core.module_manager.add_one_rank_buffer(b)
...
class ForwardAllreduceForTP(torch.autograd.Function):
    ...
    def forward(ctx, tensor, op):
        with core.nccl_throttler.throttle():
