...
if hf_transformers_available:
    ...
    def hf_gptneox_transformer_init_hook(config):
        ...
        if config.hidden_act not in ["gelu", "gelu_new", "relu"]:
            raise RuntimeError("Only 'gelu_new', 'gelu', and 'relu' activations are supported.")
        ...
        kwargs = {
            ...
            # In transformers 4.23 use_parallel_residual key was introduced to make it more configurable,
            # and compatible with different neox variants.
            # In earlier versions, the code assumed this was True.
            # see: https://github.com/huggingface/transformers/pull/18695
            # TODO: use that key when we upgrade HF version supported
                ...
                if block == "mlp":
                    ...
                    if layer == "dense":
                        ...
                        if param_type == "weight":
                            ...
                            key = param.narrow(1, attention_head_size, attention_head_size).reshape(
                                *new_shape
                            )
                            value = param.narrow(
                                1, 2 * attention_head_size, attention_head_size
                            ).reshape(*new_shape)
                        else:
    ...
    translate_hf_gptneox_state_dict_to_smdistributed = (
        lambda state_dict, max_seq_len: translate_hf_state_dict_to_smdistributed_gptneox(state_dict)
    )
    ...
    def translate_state_dict_to_hf_gptneox(state_dict, max_seq_len=None):
        ...
        for name, param in state_dict.items():
            ...
            if tokens[-3:] == ["attention", "query", "weight"]:
                layer_id = tokens[-4]
                translated_state_dict[
                    "gpt_neox.layers." + layer_id + ".attention.bias"
            ...
            if tokens[0] == "lm_head":
                ...
                raise RuntimeError(f"Unknown token {tokens[0]}.")
    ...
    def handle_qkv(qkv_list, num_attention_heads):
        ...
        for idx, (tokens, param) in enumerate(qkv_list):
            ...
            translated_name = (
                "gpt_neox.layers." + tokens[2] + ".attention.query_key_value." + tokens[5]
            )
            ...
            if tokens[4] == "query":
                ...
                raise RuntimeError(f"Unrecognized token {tokens[2]}.")
            ...
            weight_slice = qkv_dict[translated_name].narrow(0, slice_ind * slice_size, slice_size)
    ...
    def get_transformer_translation(tokens, param):
        ...
        raise RuntimeError(f"Unknown tokens {tokens}.")
